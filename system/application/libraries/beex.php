...
//	<div class='image' style='background-image: url(\"".base_url()."/media/profiles/".$row->user_id.'/cropped120_'.$row->profile_pic."\");'></div>"
	...
	function create_browser($result, $table, $folder = 'profile', $widget = false, $from_cluster = false) {
		$i = 0;
		foreach($result->result() as $item) {
			...
			if($table == 'challenges') {
				...
				<div class='row join<?php if($widget) echo " widget".$i; ?>' id="join<?php echo $item->id; ?>">
					...
					<?php if(!($widget || $from_cluster)) : ?><img class='watermark' src='<?php echo base_url(); ?>images/glyphs/join.png'><? endif; ?>
